...
import importlib.util        # Utilizado para checar disponibilidade de módulos sem importá-los globalmente
...
# Verificar disponibilidade de bibliotecas pesadas para lazy loading
HAS_GEMINI = importlib.util.find_spec('google.generativeai') is not None
HAS_TEXTSTAT = importlib.util.find_spec('textstat') is not None
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(robots_url, timeout=TIMEOUT_SECONDS) as req:
                if req.status == 200:
                    ...
                    return {"error": f"robots.txt returned {req.status}", "score_part": 0, "details": {}}
    except Exception:
        return {"error": "Could not fetch robots.txt", "score_part": 0, "details": {}}
